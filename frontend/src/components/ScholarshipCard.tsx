      ...
      <Card className={`flex flex-col h-full border-0 bg-gradient-to-br from-white to-blue-50/30 shadow-lg hover:shadow-2xl transition-all duration-300 ${className}`}>
        ...
        <CardHeader className="pb-2">
          <div className="flex justify-between items-start gap-3 mb-2">
            {/* Title: Giữ min-h để dành chỗ cho 2 dòng */}
            <CardTitle 
              className="text-lg font-semibold line-clamp-2 text-balance bg-gradient-to-r from-gray-900 to-blue-900 bg-clip-text text-transparent min-h-[3.5rem]"
          ...
          {/* --- CỤM INFO: TÁCH 2 DÒNG CỐ ĐỊNH --- */}
          <div className="flex flex-col gap-0.5"> {/* gap-0.5 để 2 dòng gần nhau hơn */}
            
            {/* DÒNG 1: Tên trường (BOLD + TRUNCATE) */}
            <div 
              className="text-sm font-bold text-gray-900 truncate" // font-bold theo yêu cầu
              title={scholarship.university || `Provider #${scholarship.providerId}`}
            >
               {scholarship.university || `Provider #${scholarship.providerId}`}
            </div>

            {/* DÒNG 2: Địa điểm (ICON + TRUNCATE) */}
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <MapPin className="h-3 w-3 shrink-0" />
              <span 
                className="truncate" 
                title={scholarship.location || "Remote"}
              >
                {scholarship.location || "Remote"}
              </span>
            </div>
        ...
        <CardContent className="flex-1 flex flex-col space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="truncate">{scholarship.studyMode?.replace('_', ' ') || 'N/A'}</span>
            </div>
            
            {(scholarship.scholarshipAmount ?? scholarship.amount ?? 0) > 0 && (
              <div className="flex items-center gap-2 col-span-2">
                <div className="w-4 h-4 rounded-full bg-gradient-to-br from-green-500 to-emerald-500 flex items-center justify-center shrink-0">
          ...
          <div className="flex items-center justify-between mt-auto pt-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className={`text-sm font-medium truncate ${deadlineStatus.color}`}>
                {t('scholarshipCard.deadline')}: {deadlineStatus.text}
              </span>
            </div>
            
            {isDeadlineSoon && (
              <Badge variant={deadlineStatus.variant} className="text-xs shrink-0 ml-2">
